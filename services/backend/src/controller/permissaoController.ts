...
export default class PermissaoController{
    ...
    static async habilitarPermissaoAgente(usuarioId: number){
        try{
            const query = 'select * from agente_usuario where usuario_id = ?';
            const values = [usuarioId];
            ...
            return new Promise<any>((resolve, reject) => {
                db.query(query, values, async (err, results) => {
                    if(err){
                        console.error('Erro ao habilitar permissão do agente:', err);
                        reject({success: false, message: 'Erro ao habilitar permissão do agente', error: err});
                    }else{
                        if((results as any[]).length == 0){
                            resolve({success: false, message: 'Usuário não possui permissão de agente'});
                        }else{
                            const queryDesabilitaPermissao = 'update agente_usuario set selecionado = 1 where usuario_id = ?';
                            db.query(queryDesabilitaPermissao, values, async (err) => {
                                if(err){
                                    console.error('Erro ao habilitar permissão do agente:', err);
                                    reject({success: false, message: 'Erro ao habilitar permissão do agente', error: err});
                                }else{
                                    resolve({success: true, message: 'Permissão de agente habilitada com sucesso'});
                                }
                            });
                        }
                    }
