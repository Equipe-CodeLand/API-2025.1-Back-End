...
class FAQClassifier:
    def __init__(self, faq_data):
        ...
        self.classifiers = {}
        self.subtopico_encoders = {}
        self.train_classifiers()

    def preprocess_text(self, text):
        # Limpeza mínima: remover pontuação e converter para minúsculas
        import re
        text = text.lower()
    ...
    def generate_embeddings(self):
        # Gerar embeddings para todas as perguntas no FAQ
        return self.encoder.encode(self.df['pergunta'].apply(self.preprocess_text).tolist())

    def train_classifiers(self):
        # 1. Classificador de Tópicos
        print("Treinando classificador de Tópicos...")
        X_top = self.embeddings
        y_top = self.df['topico']
        self.topico_encoder = LabelEncoder()
        y_top_encoded = self.topico_encoder.fit_transform(y_top)
        self.classifiers['topico'] = SVC(probability=True)
        self.classifiers['topico'].fit(X_top, y_top_encoded)

        # Avaliação do classificador de Tópicos
        y_pred_top = self.classifiers['topico'].predict(X_top)
        print("--- Avaliação do classificador de Tópicos ---")
        print(f"Acurácia: {accuracy_score(y_top_encoded, y_pred_top):.2f}")
        print("Relatório de Classificação:")
        print(classification_report(y_top_encoded, y_pred_top, target_names=self.topico_encoder.classes_))
        print("Matriz de Confusão:")
        print(confusion_matrix(y_top_encoded, y_pred_top))

        # 2. Classificadores de Subtópicos
        for topico in self.df['topico'].unique():
            print(f"Treinando classificador de Subtópicos para '{topico}'...")
            subset = self.df[self.df['topico'] == topico]
            X_sub = self.embeddings[subset.index]
            y_sub = subset['subtopicos'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
            self.subtopico_encoders[topico] = LabelEncoder()
            y_sub_encoded = self.subtopico_encoders[topico].fit_transform(y_sub)
            self.classifiers[f'subtopico_{topico}'] = SVC(probability=True)
            self.classifiers[f'subtopico_{topico}'].fit(X_sub, y_sub_encoded)

            # Avaliação do classificador de Subtópicos
            y_pred_sub = self.classifiers[f'subtopico_{topico}'].predict(X_sub)
            print(f"--- Avaliação do classificador de Subtópicos ({topico}) ---")
            print(f"Acurácia: {accuracy_score(y_sub_encoded, y_pred_sub):.2f}")
            print("Relatório de Classificação:")
            print(classification_report(y_sub_encoded, y_pred_sub, target_names=self.subtopico_encoders[topico].classes_))
            print("Matriz de Confusão:")
            print(confusion_matrix(y_sub_encoded, y_pred_sub))

        # 3. Classificador de Dificuldade (se existir)
        if 'dificuldade' in self.df.columns:
            print("Treinando classificador de Dificuldade...")
            X_diff = self.embeddings
            y_diff = self.df['dificuldade']
            self.dificuldade_encoder = LabelEncoder()
            y_diff_encoded = self.dificuldade_encoder.fit_transform(y_diff)
            self.classifiers['dificuldade'] = SVC(probability=True)
            self.classifiers['dificuldade'].fit(X_diff, y_diff_encoded)

            # Avaliação do classificador de Dificuldade
            y_pred_diff = self.classifiers['dificuldade'].predict(X_diff)
            print("--- Avaliação do classificador de Dificuldade ---")
            print(f"Acurácia: {accuracy_score(y_diff_encoded, y_pred_diff):.2f}")
            print("Relatório de Classificação:")
            print(classification_report(y_diff_encoded, y_pred_diff, target_names=self.dificuldade_encoder.classes_))
            print("Matriz de Confusão:")
            print(confusion_matrix(y_diff_encoded, y_pred_diff))

    def classify_question(self, question):
        # Pré-processamento da pergunta
        question_clean = self.preprocess_text(question)
        embedding = self.encoder.encode([question_clean])

        # 1. Classificar tópico
        topico_idx = self.classifiers['topico'].predict(embedding)[0]
        topico = self.topico_encoder.inverse_transform([topico_idx])[0]
        topico_conf = np.max(self.classifiers['topico'].predict_proba(embedding))
        ...
        # 3. Classificar dificuldade (se existir)
        dificuldade = None
        if 'dificuldade' in self.classifiers:
            dificuldade_idx = self.classifiers['dificuldade'].predict(embedding)[0]
            dificuldade = self.dificuldade_encoder.inverse_transform([dificuldade_idx])[0]
    ...
    def find_similar_questions(self, question, n=3, threshold=0.6):
        # Buscar perguntas semelhantes
        question_clean = self.preprocess_text(question)
        embedding = self.encoder.encode([question_clean])
        similarities = cosine_similarity(embedding, self.embeddings)[0]
        ...
        if len(top_indices) == 0 or max(similarities) < threshold:
            return pd.DataFrame()

        similar = self.df.iloc[top_indices].copy()
        similar['similaridade'] = similarities[top_indices]
        return similar
...
if __name__ == "__main__":
    # Carregar dados e inicializar o sistema
    faq_data = carregar_faq_json("./data/data1.json")
    ...
    while True:
        try:
            ...
            if pergunta.lower() == 'sair':
                print("Encerrando o sistema...")
                break
            resposta = faq_classifier.get_answer(pergunta)
